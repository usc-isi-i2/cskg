    ...
    for rel in rels:
        ...
        for sub_name in sub_names:
            ...
            for obj_name in obj_names:
                ...
                rel_edge=[pred_id, create_uri(VG_NS, 'subject'), sub_id, data_source, weight, json.dumps({'image_id': image_id})]
                all_edges.append(rel_edge)

                rel_edge=[pred_id, create_uri(VG_NS, 'object'), obj_id, data_source, weight, json.dumps({'image_id': image_id})]
                all_edges.append(rel_edge)

        pos=''
        for s in synsets:
            rel_syn_col=[pred_id, WORDNET_SENSE_REL, create_uri(WORDNET_NS, s), data_source, weight, json.dumps({'image_id': image_id})]
        ...
        rel_node=[pred_id, label, ','.join(aliases), pos, data_source, json.dumps({'image_id': image_id})]
        all_nodes.append(rel_node)

        rel_img_edge=[pred_id, INIMAGE_REL, image_node, data_source, weight, json.dumps({})]
                ...
                for attr in o['attributes']:
                    ...
                    if attr in attr_synsets:
                        ...
                        attr_wn_edge=[a_id, WORDNET_SENSE_REL, create_uri(WORDNET_NS, a_synset),
                              data_source, weight, json.dumps({'image_id': image_id})]
                    ...
                    attr_node=[a_id, a_label, ','.join(a_aliases), a_pos, data_source, json.dumps({'image_id': image_id})]
                    all_nodes.append(attr_node)

                    # edge from object to an attribute
                    obj_attr_edge=[o_id, HAS_PROPERTY_REL, a_id, data_source, weight, json.dumps({'image_id': image_id})]
                    all_edges.append(obj_attr_edge)


            obj_node=[o_id, name, '', o_pos, data_source, json.dumps({'image_id': image_id})]
            all_nodes.append(obj_node)

        obj_img_edge=[o_id, INIMAGE_REL, image_node, data_source, weight, json.dumps({})]
...
# OUTPUT FILES
output_dir='../output_v%s/visualgenome-lbl' % VERSION
...
for counter, an_image in enumerate(images_data):    
    ...
    image_node=create_uri(VG_NS, 'I' + image_id)
    image_node_entry=[image_node, '', '', '', data_source, json.dumps({})]
...
for synset, labels in wn2label.items():
    ...
    wn_node=[create_uri(WORDNET_NS, synset), label, ','.join(aliases), pos, data_source, json.dumps({'image_ids': images})]
